...
test('[09/part_1] should work', async () => {
  const DAY = 'd09'
  const PART_1 = 'part-1'
  const EXPECTED = 6311
  const answer = await main(DAY, PART_1)
...
test('[09/part_2] should work', async () => {
  const DAY = 'd09'
  const PART_2 = 'part-2'
  const EXPECTED = 201684
  const answer = await main(DAY, PART_2)
...
test('[10/part_1] should work', async () => {
  const DAY = 'd10'
  const PART_1 = 'part-1'
...
test('[10/part_2] should work', async () => {
  const DAY = 'd10'
  const PART_2 = 'part-2'
  const EXPECTED = `\n####.#..#.###..#..#.####.###..#..#.####.\n#....#.#..#..#.#..#.#....#..#.#..#....#.\n###..##...#..#.####.###..#..#.#..#...#..\n#....#.#..###..#..#.#....###..#..#..#...\n#....#.#..#.#..#..#.#....#....#..#.#....\n####.#..#.#..#.#..#.####.#.....##..####.`
  const answer = await main(DAY, PART_2)
...
test('[11/part_1] should work', async () => {
  const DAY = 'd11'
  const PART_1 = 'part-1'
  const EXPECTED = 55944
  const answer = await main(DAY, PART_1)
  if (answer !== EXPECTED) {
    throw new error('E_WRONG_ANSWER')
  }
...
test('[11/part_2] should work', async () => {
  const DAY = 'd11'
  const PART_2 = 'part-2'
  const EXPECTED = 15117269860
  const answer = await main(DAY, PART_2)
  if (answer !== EXPECTED) {
    throw new error('E_WRONG_ANSWER')
  }
