...
const drawMap = (lines: Array<Line>) => {
  ...
  lines.forEach(({ start: [xStart, yStart], end: [xEnd, yEnd] }) => {
    ...
    if (xStart !== xEnd && yStart !== yEnd) {
      ...
      const fromY = fromX === xStart ? yStart : yEnd
      const toX = Math.max(xStart, xEnd)
      const toY = toX === xStart ? yStart : yEnd
      let ascending = false
      if ((fromX === xStart && yStart < yEnd) || (fromX === xEnd) && yEnd < yStart) {
        ascending = true
      }
      let i = 0
      while (i <= Math.abs(fromY - toY)) {

        const cursorY = fromY + (i * (ascending ? 1 : -1))
        map[cursorY][fromX + i] = ['⬜️', '🟧'].includes(map[cursorY][fromX + i]) ? '🟧' : '⬜️'
        i++
