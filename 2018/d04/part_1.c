...
static int ft_search_laziest(t_report **report)
{
	t_report 	*tmp_report;
	int				lazy_time;
	int				lazy_id;
	...
	while (tmp_report)
	{
		if (tmp_report->asleep_tot > lazy_time)
		{
			lazy_time = tmp_report->asleep_tot;
			lazy_id = tmp_report->id;
		}
		tmp_report = tmp_report->next;
	}
	return (lazy_id);
...
int	main (void)
{
	...
	// Then we build from this list a list of unical guard whith an associeted asleep time counter
	ft_build_report(&list, &report); 	// So far from here, we're able to determine who has slept the most

	// Then we build a timesheet for the laziest guard  to determine
	// on each minute he was the most asleep
	lazy_id = ft_search_laziest(&report);
	timesheet = ft_build_timesheet(&list, lazy_id);

	// We find this minute
	i = -1;
	sneak_moment = 0;
	while (++i < 60)
		sneak_moment = (timesheet[i] > timesheet[sneak_moment]) ? i : sneak_moment;

	// At the end, we print the answer
	ft_putnbr(sneak_moment * lazy_id);
	ft_putendl("");
	return(0);
}
