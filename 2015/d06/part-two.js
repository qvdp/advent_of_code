    ...
    if (!entries[i]) {
      if (currentGoup) {
        ...
        groups.push({
          answers,
          yesAnswers,
          yesGroup: yesAnswers.filter((letter) => answers.every((form) => form.includes(letter))),
          yesCount: (yesAnswers.filter((letter) => answers.every((form) => form.includes(letter)))).length
        })
      }
      currentGoup = ''
    } else {
